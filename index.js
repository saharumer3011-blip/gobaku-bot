...
// Writes synchronously and immediately -- no debounce. This bot's scale
// (a handful of chats/messages at a time) makes the I/O cost negligible,
// and a debounced/deferred write left a real gap: if a redeploy's shutdown
// signal arrived between a state change and the deferred write actually
// flushing, that change was silently lost, causing the bot to "forget"
// where a conversation was and repeat itself after the very next deploy.
// Writing synchronously means every state change is durable on disk before
// the webhook handler even finishes responding.
...
// should be a no-op in practice -- but it costs nothing to be sure nothing
// is lost in the shutdown window.
function shutdown(signal) {
  console.log(`Received ${signal}, flushing state before exit...`);
  persistState();
  process.exit(0);
}
