...
//
// WHY THIS EXISTS: eventId dedup only catches *exact* repeat deliveries.
// WhatsApp multi-device / Blueticks can also deliver two DIFFERENT event IDs
// for what is effectively the same incoming customer message. When that
// happens, two webhook requests for the same chatId can run
// handleIncomingMessage() concurrently. Since state flags (sentMenu,
// sentPackageNumber, etc.) are only set to true AFTER the outgoing
// sendText/sendMedia call finishes awaiting, both concurrent calls can read
// the flag as still false and both send the package menu / itinerary —
// this is what caused the duplicate package list in production.
//
// Fix: force every webhook for a given chatId to run one-at-a-time, in
// order, by chaining onto a per-chat promise queue.
// ---------------------------------------------------------------------------
const chatQueues = new Map();

function runSerializedForChat(chatId, task) {
  const previous = chatQueues.get(chatId) || Promise.resolve();
  const result = previous.then(task, task); // run task even if prior one errored
  // Keep the chain alive but swallow errors so the Map entry doesn't hold a
  // rejected promise forever (which would break every future .then() on it).
  chatQueues.set(
    chatId,
    result.catch(() => {})
  );
...
function getState(chatId) {
  if (!chatState.has(chatId)) {
    chatState.set(chatId, {
      ...
      lastMessageAt: null, // epoch ms of the last inbound message we handled
    });
  }
  return chatState.get(chatId);
}

// How long a chat has to go quiet before a "fresh inquiry" keyword is
// trusted to mean the customer restarted the conversation (vs. just asking
// a normal follow-up mid-chat that happens to contain a common word like
// "package" or "detail").
const FRESH_INQUIRY_GAP_MS = 6 * 60 * 60 * 1000; // 6 hours

function looksLikeRestartedConversation(state, now) {
  if (!state.sentMenu) return true; // never sent it yet — not a "resend"
  if (!state.lastMessageAt) return true; // no record of prior activity
  return now - state.lastMessageAt >= FRESH_INQUIRY_GAP_MS;
}
...
async function handleIncomingMessage(chatId, messageType, text) {
  ...
  const now = Date.now();
  // Snapshot "was this a stale/restarted conversation?" BEFORE we touch
  // lastMessageAt below, since the fresh-inquiry check further down needs
  // the gap since the *previous* message, not this one.
  const isRestartedConversation = looksLikeRestartedConversation(state, now);
  ...
  // Only treat "package"/"detail"/"baku"/etc. as a signal to resend the
  // WHOLE menu if the conversation actually looks restarted (long gap since
  // we last heard from them, or we've never sent the menu at all). These
  // words are extremely common in ordinary follow-up questions once the
  // menu has already been sent ("what's in package 3?", "more detail on
  // Baku hotels?"), so without the gap check every such question was
  // wrongly re-blasting the full menu mid-conversation.
  const freshInquiryTrusted =
    isFreshInquiry(text) && !state.sentPackageNumber && isRestartedConversation;
...
app.post("/webhook", async (req, res) => {
  try {
    ...
    // Serialized per chatId so two near-simultaneous webhook deliveries for
    // the same conversation (different eventIds, same underlying message)
    // can never race each other and both fire off the package menu / PDF.
    await runSerializedForChat(chatId, () => handleIncomingMessage(chatId, type, text));
    res.sendStatus(200);
  } catch (err) {
