...
async function handleIncomingMessage(chatId, messageType, text) {
  ...
  // Guarded against short conversational replies ("yes please", "thanks",
  // etc.) -- these clearly reply to *something*, so blasting the full
  // package menu back is wrong/confusing even if our state thinks sentMenu
  // is false (e.g. after a state reset). Safer to flag those for a human.
  if ((!state.sentMenu || isFreshInquiry(text)) && !isShortConversationalReply(text)) {
    await sendText(chatId, PACKAGE_MENU);
    state.sentMenu = true;
    await markRead(chatId);
    return;
