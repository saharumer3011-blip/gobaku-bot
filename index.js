...
async function handleIncomingMessage(chatId, messageType, text) {
  ...
  if (state.awaitingTravelInfo) {
    ...
    // From this point on, the bot goes silent in this chat permanently
    // (even for follow-up visa/ticket/pricing questions) -- the client has
    // been told a human team will follow up, so any further auto-replies
    // would be confusing or contradict what a human ends up saying. A human
    // can still manually resume the bot for this chat from the admin page
    // if that's ever actually wanted.
...
app.post("/webhook", async (req, res) => {
  try {
    ...
    if (fromMe) {
      // This is an outgoing message -- either the bot's own automated
      // reply, or a human agent typing directly in WhatsApp Web. Tell them
      // apart via the tracked message-ID set (see botSentMessageIds above).
